...
test(
  "GET /api/ledger/jars returns the fields the JarCard UI consumes",
  async () => {
...
test(
  "GET /api/ledger/transactions?accountId=<jarId> returns the jar's transactions for the latest-movement row",
  async () => {
    await withTempLedger(async ({ tempRoot }) => {
      const server = createLocalApiServer({
