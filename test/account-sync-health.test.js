...
test(
  "snapshot.webhookEvents is filterable by accountId and status for the E5 sync health section",
  async () => {
...
test(
  "snapshot.summary.lastSyncedAt and oldestSyncCursorUpdatedAt are strings when present",
  async () => {
    await withTempLedger(async ({ tempRoot }) => {
      const server = createLocalApiServer({
        profile: "e5-summary",
