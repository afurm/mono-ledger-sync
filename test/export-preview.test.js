...
test(
  "GET /api/ledger/transactions returns the total the export preview shows",
  async () => {
...
test(
  "GET /api/ledger/transactions supports from/to date range filters for the export preview",
  async () => {
    await withTempLedger(async ({ tempRoot }) => {
      const server = createLocalApiServer({
        profile: "g3-range",
